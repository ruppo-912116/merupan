...
export default function Overview({activeSection}: { activeSection: string }) {
    ...
    const SocialMedias = [
        ...
        },
        {
            ...
            >
                <path
                    d="M0 4.35V3.94L2.16 0h.38l3.45 6.54L9.24 0h.37l2.16 3.94v.41L9.87 8l1.9 3.65v.4l-2.16 3.95h-.38L6 9.46 3.45 16h-.38L0 12.05v-.4L1.9 8 0 4.35z"/>
            </svg>
        }
    ]
                            ...
                            <li>
                                <Link href={`#${section.toLowerCase()}`} className={"group flex items-center py-3"}>
                                    <span
                                        ...
                                        ${activeSection === section.toLowerCase() ? "w-16 bg-slate-200" : "bg-slate-600 group-hover:w-16 group-hover:bg-slate-200 group-focus-visible:w-16group-focus-visible:bg-slate-200"} 
                                         motion-reduce:transition-none`}>
                                    </span>
                                    <span
                                        className={`nav-text text-xs font-bold uppercase tracking-widest 
                                        ${activeSection === section.toLowerCase() ? "text-slate-200"
